...
\begin{enumerate}
  \item \textbf{Load} the practice's rows. In the pilot this is the practice's own Helios FHIR server or
  ...
  \item \textbf{Reply} with the updated coefficient vector and \texttt{num-examples}.
        ...
        The model is logistic regression over \NumFeatures{} features plus an intercept: age, six
        binary comorbidity flags, and three ``years since diagnosis'' durations.
        \S\ref{sec:modelchoice} explains why this model class rather than a stronger one, and why
        that choice is also the privacy-preferable one.
\end{enumerate}
...
weighted by it, and for no other purpose. \textbf{Aggregation is the last step of a round, not a
property of transmission}, and \textbf{every message carries its sender's identity}.
