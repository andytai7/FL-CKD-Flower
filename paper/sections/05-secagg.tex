...
practices and the server obtains only the sum. It also composes with the record-level DP-SGD standard
of \S\ref{sec:noise}: a live run of \texttt{dpsgd-epsilon} \emph{together with}
\texttt{secure-aggregation=true} drove the census round, the plan dispatch and all planned rounds over
the masked rails and completed 12/12 results with zero practice failures. Three limits must travel
with any claim made about it. The guarantee is \textbf{semi-honest} it protects against a server that follows the protocol
...
legacy context. The composition problem this used to pose is resolved: the DP-SGD standard's server
agent drives the legacy SecAgg+ workflow from inside the modern server application, so the
orchestrated census round, the per-practice plan dispatch and the planned training rounds all travel
over the masked rails verified live with zero failures, as above. Two combinations remain excluded
by construction, and the run configuration enforces both: \texttt{central-dp-epsilon} with
\texttt{secure-aggregation} (server-side noise over an opaque sum would be meaningless), and
\texttt{dpsgd-epsilon} with either \texttt{central-dp-epsilon} or \texttt{local-dp-epsilon} (one
noise mechanism per run).
