...
correct. The two alternatives are materially worse: the tree-based protocol transmits
\UplinkXGB{} bits per round whose split thresholds are \emph{literal patient feature values}, which
is the highest-disclosure payload in the comparison and is also the one secure aggregation cannot
protect.
\end{shortanswer}
...
\begin{figure}[htbp]
  ...
  \caption{Uplink volume per practice per round, log scale. The tree-based protocol costs roughly
  sixty times the coefficient vector and discloses more per bit.}
  \label{fig:payload}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Serialized decision trees} (FedXgbBagging) are categorically different. A split
