...
> **Branch charter (research/privacy-dl-ts).** This document is the contract for the
> deep-learning × privacy benchmark program. It lives on this branch because it **requires a
> branch-local relaxation of CLAUDE.md rule 2** (logreg-only): LSTM/GRU/PatchTST are permitted
> **here and only here**, by user direction (2026-09-04). Rules 1 and 8 still bind: all federation
> runs through Flower's real `ServerApp`/`ClientApp` and every protocol below is a `Strategy`
> subclass — no hand-rolled aggregation, no custom client/server transport. Nothing from this
> branch merges to `main`/`dev` without a consortium-level re-scoping of rule 2.
>
...
| Forecasting | **ETTh1 / ETTm1** (Informer release) | 96/192/336-step oil-temperature forecasting | the canonical PatchTST-vs-LSTM testbed; univariate target, multivariate covariates |
...
### Model suite (shared interface)

One factory, three families, all exposing `(params: StateDict) -> torch.Tensor` flattening so the
...
class LSTMClassifier(nn.Module):    # hidden 64, 2 layers, mean-pool over time -> logit
class GRUForecaster(nn.Module):     # seq2seq decoder, horizon H
class PatchTST(nn.Module):          # Nie et al. 2023: patch_len 16, stride 8, 3 layers,
