...
### Phase 0: Design Interview (MANDATORY)

**Do not skip this phase and do not proceed to Phase 1 until it passes.**

Your number one job while planning is to **reduce the number of assumptions the implementation agent will make**. A plan built on unasked questions produces confident, wrong code. So, before any analysis:

1. Check whether a signed Decision Record already exists:
   - If a `docs/decisions/*.md` file was provided (an argument, a prior
     `design-interview`, or the plan is being authored from one), read it fully
     and treat its `## Decisions`, `## Delegated decisions` and
     `## Validation plan` sections as **binding input**.
   - If **no** decision record exists, run the **`design-interview` skill**
     first. It interviews the user using the `ask` tool and writes
     `docs/decisions/YYYY-MM-DD-<slug>.md`. Do not invent alternatives later.
2. If the record's `## Open Questions` is not `_None_`, STOP: surface the open
   questions to the user and re-run the interview. Never guess.
3. Carry the decisions through the whole plan. If codebase research in a later
   phase contradicts a recorded decision, STOP and re-interview that item
   instead of silently deviating.

The Decision Record and this plan are two separate artifacts with different
lifetimes: the record is the durable "why", this plan is the "how" for one
execution. Pointed at by [REVIEW ITEMS] below.

Every plan MUST include a `## Decision Record` section (see template) stating
the decisions the implementation is bound by, and MUST list any
`**UNRESOLVED**` item explicitly rather than papering over it.
...
The implementation MUST follow these decisions exactly. If any decision
conflicts with codebase reality, STOP and re-interview — do not deviate.

- **Source:** `docs/decisions/YYYY-MM-DD-<slug>.md` (as produced by the
  `design-interview` skill; copy the `## Decisions` table here)
- **Decisions:**
  1. <topic>: <decision> (Source: User/Delegated)
  2. ...
- **Delegated decisions (agent-owned):** <list, if any>
- **Explicitly out of scope:** <list>
- **UNRESOLVED (must be empty):** _None._ Or list open items — the execution
  agent must NOT start if this is non-empty.
