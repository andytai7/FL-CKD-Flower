...
```bash
# in the pack, the source is under .agents/; installed repos get both layouts:
cp .agents/settings.json.example .claude/settings.json   # in a target repo
```

`install.sh` copies the template into every repo (writing a live
`.claude/settings.json` when the repo has none). `.claude/settings.json` is gitignored here in the pack so
the hooks don't fire while you're reading the material. **In your own project, commit it** — that's how the
whole team inherits the same guarantees. If you already have a `settings.json`, merge the `hooks` block in
rather than overwriting it.

OMP note: this pack's hooks are the Claude Code `settings.json` shape. The OMP harness runs hooks as
`.omp/` extensions — same mental model (pre = gate, post = log), different file — see the Portability note
below.
